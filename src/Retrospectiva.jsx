...
// Lê um arquivo GPX (Strava/Garmin) e devolve o traçado já projetado e simplificado,
// pronto pra desenhar: { pts:[[x,y],...], w, h }. Projeção equiretangular com correção
// de latitude (lon·cos(lat)); norte pra cima; caixa normalizada (~1000 no maior lado).
// Amostra pra no máx ~250 pontos, pra não inchar o sync. null se não for um GPX válido.
...
// Desenha o traçado de uma prova (saída do parseGpx) como uma linha SVG, com um ponto
// verde na largada e um vermelho na chegada.
function RotaChart({ rota }) {
  ...
  return (
    <svg viewBox={`${-pad} ${-pad} ${w + pad * 2} ${h + pad * 2}`} style={{ width: '100%', maxHeight: 220, height: 'auto', display: 'block' }}>
      <path d={d} fill="none" stroke={COR_CORRIDA} strokeWidth={sw} strokeLinejoin="round" strokeLinecap="round" opacity="0.9" />
...
// Bloco do trajeto num card de prova: desenha (se houver) + botão pra subir/trocar/remover
// o GPX. Salva no exercício original (campo `rota`), sem os campos derivados do card.
function RotaProva({ prova }) {
  ...
  const onFile = (e) => {
    ...
    reader.onload = () => {
      const rota = parseGpx(String(reader.result || ''));
      if (!rota) { setErro('Não consegui ler esse arquivo. Confira se é o .gpx exportado do Strava/Garmin.'); return; }
      setErro('');
      cal.saveExercicio({ ...orig, rota });
    };
    reader.onerror = () => setErro('Não consegui abrir o arquivo.');
    reader.readAsText(f);
  };
  const remover = () => { if (!orig) return; const { rota, ...rest } = orig; cal.saveExercicio(rest); };
  const linkBtn = { background: 'none', border: 'none', padding: 0, cursor: 'pointer', fontSize: 12, fontWeight: 700, color: COR_CORRIDA };
  return (
    <div style={{ marginTop: 10 }}>
      {prova.rota && (
        <div style={{ background: '#faf7f5', border: '1px solid #f1e7e3', borderRadius: 10, padding: 8, marginBottom: 6 }}>
          <RotaChart rota={prova.rota} />
        </div>
      )}
      <div style={{ display: 'flex', gap: 14, alignItems: 'center' }}>
        <button onClick={() => inputRef.current && inputRef.current.click()} style={linkBtn}>{prova.rota ? '↻ trocar trajeto' : '＋ adicionar trajeto (GPX)'}</button>
        {prova.rota && <button onClick={remover} style={{ ...linkBtn, color: '#bbb' }}>remover</button>}
